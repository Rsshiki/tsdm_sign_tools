...
def main():
    ...
    for username, account_info in accounts.items():
        ...
        try:
            logger.info(f"开始处理账号 {username} 的任务")
            driver, user_data_dir = setup_driver(headless=True)
            if driver:
                logger.info(f"浏览器已启动，开始访问主页面")
                driver.get('https://www.tsdm39.com')
                logger.info(f"开始为账号 {username} 添加 cookie")
                for cookie in account_info["cookies"]:
                    driver.add_cookie(cookie)
                logger.info(f"cookie 添加完成，准备查看账号 {username} 的签到情况")
            ...
            logger.error(f"处理账号 {username} 时发生错误: {e}")
        finally:
            # 关闭浏览器
            if driver:
                try:
    ...
    # 提示 cookie 过期的账号
    if expired_accounts:
        logger.info("以下账号的 cookie 已过期，请重新登录获取新的 cookie:")
    ...
    for info in account_time_list:
        ...
        logger.info(f"账号 {username} 在 {last_work_time} 打工完成，下次可打工时间 {next_work_time}")
    ...
    if latest_next_work_time:
        logger.info(f"创建下次打工计划任务，预计在 {latest_next_work_time.strftime('%H:%M:%S')} 后执行")
        create_scheduled_task(latest_next_work_time)

    create_login_startup_task()
    logger.info("所有任务完成，程序将在 5 秒后自动退出...")
